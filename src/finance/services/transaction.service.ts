...
export class TransactionService extends Repository implements ITransactionService<Transaction> {
  ...
  try {
    ...
    if (transaction === false  && !transaction)  throw new Error('Transaction not found');
    return true;
    } catch (error) {
